...
public class DonutProgressView: UIView{
    ...
    func updateData(data: [Datasource]){
        self.progressLayers.forEach({$0.removeFromSuperlayer()})
        let padding = CGFloat(data.filter({$0.percent > 0}).count) * 0.17
        let totalRotationAngle = (CGFloat.pi * 2) - padding
        var startAngle = CGFloat.zero//CGFloat.initialAngle
        ...
        for obj in data{
            if obj.percent > 0 {
                ...
                let path = UIBezierPath(arcCenter: center, radius: radius, startAngle: startAngle, endAngle: startAngle + progressAngle, clockwise: true)
...
public class CircleProgressView: UIView {
    ...
    var progress: CGFloat = 0.0{
        didSet{
            if isWithAnimation{
                self.animateCircle(duration: 1, delay: 0)
            }else{
    ...
    @IBInspectable var selectedDarkLineColor: UIColor? = .blue{
        didSet{
            ...
            self.upperProgressLayer.strokeColor = selectedDarkLineColor.cgColor
        }
    }
    ...
    @IBInspectable var lineWidth: CGFloat = 10 {
        didSet{
            ...
            self.upperProgressLayer.lineWidth = lineWidth
        }
    }
    ...
    public func setupProgress(progress: CGFloat){
        ...
        if progress > 1 {
            ...
            let _progressPath = UIBezierPath(arcCenter: center, radius: radius, startAngle: .initialAngle, endAngle: .endAngle(progress: _progress), clockwise: true)
            upperProgressLayer.path = _progressPath.cgPath
            upperProgressLayer.strokeEnd = 1//_progress
