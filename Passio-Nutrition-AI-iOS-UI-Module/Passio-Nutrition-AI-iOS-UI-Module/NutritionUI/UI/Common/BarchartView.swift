...
class CustomBarChartView: UIView{
    ...
    func setupbarChart(dataSource: [ChartDataSource],baseLine: CGFloat? = nil, maximum: CGFloat){
        ...
        for i in 0..<dataSource.count{
            ...
            if let layer = drawLine(points: [startPoint,endPoint], strokeColor: dataSource[i].color, linewidth: self.barWidth){
    ...
    func setupLineChart(dataSource: [ChartDataSource],baseLine: CGFloat? = nil,  maximum: CGFloat, dotSize: CGFloat = 4 , lineWidth: CGFloat = 2) {
        self.layoutIfNeeded()
        self.drawGrid(gridCount: self.numberOfGrids)
        if let baseLine = baseLine {
            self.drawBaseline(value: baseLine, max: maximum)
        ...
        for i in 0..<dataSource.count{
            if let value = dataSource[i].value{
                ...
                if let dot = drawDot(points: point, strokeColor: dataSource[i].color, radius: dotSize){
        ...
        if let layer = drawLine(points: points, strokeColor: dataSource.first?.color ?? .blue, linewidth: lineWidth){
            self.layer.addSublayer(layer)
            self.barLayers.append(layer)
        }
    }
    ...
    func setupCombinedBarChart(dataSource: [CombineChartDataSource],baseLine: CGFloat? = nil, maximum: CGFloat){
        self.layoutIfNeeded()
        self.drawGrid(gridCount: self.numberOfGrids)
        if let baseLine = baseLine {
            self.drawBaseline(value: baseLine, max: maximum)
        ...
        for i in 0..<dataSource.count{
            ...
            for j in 0..<dataSource[i].dataSource.count{
                ...
                let startPoint = CGPoint(x: centerX, y: y)
                let endPoint = CGPoint(x: centerX, y: y - barHeight)
                if let layer = drawLine(points: [startPoint,endPoint], strokeColor: obj.color, linewidth: self.barWidth){
                    self.layer.addSublayer(layer)
                    self.barLayers.append(layer)
                }
                y -= barHeight
    ...
    private func drawGrid(gridCount: Int){
        ...
        for i in 0..<gridCount {
            ...
            if let layer = drawLine(points: [startPoint,endPoint], strokeColor: self.gridLineColor, linewidth: 1){
                self.layer.addSublayer(layer)
    ...
    private func drawDot(points: CGPoint,strokeColor: UIColor, radius: CGFloat = 3) -> CAShapeLayer? {
        let path = UIBezierPath(arcCenter: points, radius: radius - 1, startAngle: 0, endAngle: 2 * .pi, clockwise: true)
        return drawALayer(path, strokeColor, strokeColor ,1)
    }
    
    private func drawALayer(_ path: UIBezierPath, _ strokeColor: UIColor, _ fillColor: UIColor, _ linewidth: CGFloat) -> CAShapeLayer{
    ...
    private func drawDashedLine(points: [CGPoint], strokeColor: UIColor, linewidth: CGFloat , dashedPattern: [NSNumber] = [7,7]) -> CAShapeLayer?{
        guard let path = pathFrom(points: points) else { return nil }
