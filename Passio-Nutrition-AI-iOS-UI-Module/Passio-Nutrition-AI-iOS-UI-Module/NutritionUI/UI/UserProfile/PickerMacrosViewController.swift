...
class PickerMacrosViewController: UIViewController{
    ...
    private func setUIToMatchMacros() {
        self.caloriesValueTextField.text = "\(modelMacros.caloriesTarget)"
        
        self.carbsValueLabel.text = "\(modelMacros.carbsGrams) \(Localized.gramUnit)"
        self.carbsPercentTextField.text = "\(modelMacros.carbsPercent)"
        self.carbsSlider.value = CFloat(modelMacros.carbsPercent)
        
        self.proteinValueLabel.text = "\(modelMacros.proteinGrams) \(Localized.gramUnit)"
        self.proteinPercentTextField.text = "\(modelMacros.proteinPercent)"
        self.proteinSlider.value = CFloat(modelMacros.proteinPercent)
        
        self.fatValueLabel.text = "\(modelMacros.fatGrams) \(Localized.gramUnit)"
        self.fatPercentTextField.text = "\(modelMacros.fatPercent)"
        self.fatSlider.value = CFloat(modelMacros.fatPercent)
...
extension PickerMacrosViewController:UITextFieldDelegate{
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        let newText = (currentText as NSString).replacingCharacters(in: range, with: string).replacingOccurrences(of: "%", with: "")
