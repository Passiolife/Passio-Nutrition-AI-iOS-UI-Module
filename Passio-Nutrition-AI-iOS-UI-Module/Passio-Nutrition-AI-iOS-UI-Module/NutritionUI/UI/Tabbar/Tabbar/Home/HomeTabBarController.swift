    ...
    override func viewDidLoad() {
        ...
        for i in 0..<tabs.count {
            ...
            case .home:
                ...
                let dashboardNavVC = UINavigationController.init(rootViewController: dashboardVC)
            ...
            case .diary:
                ...
                let macroNavVC = UINavigationController.init(rootViewController: diaryVC)
    ...
    @objc private func handleFilterButton(sender: UIButton) {
        ...
        if let frame = sender.superview?.convert(sender.frame, to: nil) {
            customPickerViewController.pickerFrame = CGRect(x: frame.origin.x - 130,
                                                            ...
                                                            height: 42.5 * CGFloat(options.count))
        }
...
extension HomeTabBarController: PlusMenuDelegate {
    ...
    func onScanSelected() {
        let vc = NutriationUICoordinator.getScanningViewController()
...
extension HomeTabBarController: CustomPickerSelectionDelegate {
    ...
    func onPickerSelection(value: String, selectedIndex: Int, viewTag: Int) {
        let item = HemburgarMenuOptions.init(rawValue: value)
        switch item {
        case .profile:
            let profileVC = NutriationUICoordinator.getEditProfileViewController()
